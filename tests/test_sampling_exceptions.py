...
@pytest.mark.asyncio
async def test_sampling_index_error(api_url: str) -> None:
    """Test sampling from a model where a (Stan) index error occurs.

    Sampling from this model should generate an exception.
    """
    ...
    # verify an error occurred
    assert operation["result"]["code"] == 400
    assert "accessing element out of range" in operation["result"]["message"]
    ...
    error_message = messages[0].feature[0].string_list.value[0]
    assert "Unrecoverable error" in error_message

...
HTTP_ARCHIVES_EXPANDED := build/protobuf-$(PROTOBUF_VERSION)/.dirstamp build/stan-$(STAN_VERSION)/.dirstamp build/math-$(MATH_VERSION)/.dirstamp
...
###############################################################################
build/archives/.dirstamp:
	@mkdir -p build/archives && touch $@

$(PROTOBUF_ARCHIVE): build/archives/.dirstamp
	@echo downloading $@
...
$(STAN_ARCHIVE): build/archives/.dirstamp
	@echo downloading $@
	@curl --silent --location https://github.com/stan-dev/stan/archive/v$(STAN_VERSION).tar.gz -o $@

$(MATH_ARCHIVE): build/archives/.dirstamp
	@echo downloading $@
	@curl --silent --location https://github.com/stan-dev/math/archive/v$(MATH_VERSION).tar.gz -o $@

build/protobuf-$(PROTOBUF_VERSION)/.dirstamp: $(PROTOBUF_ARCHIVE)
build/stan-$(STAN_VERSION)/.dirstamp: $(STAN_ARCHIVE)
build/math-$(MATH_VERSION)/.dirstamp: $(MATH_ARCHIVE)
...
httpstan/include/google: build/protobuf-$(PROTOBUF_VERSION)/src/google build/protobuf-$(PROTOBUF_VERSION)/.dirstamp
	@mkdir -p httpstan/include
	@rm -rf $@
	cp -r $< $@

httpstan/lib/libprotobuf-lite.so httpstan/bin/protoc: build/protobuf-$(PROTOBUF_VERSION)/.dirstamp
...
httpstan/include/stan: build/stan-$(STAN_VERSION)/.dirstamp
	@mkdir -p httpstan/include
	@rm -rf $@
...
build/stan-$(STAN_VERSION)/src/stan: build/stan-$(STAN_VERSION)/.dirstamp

httpstan/include/stan/math: build/math-$(MATH_VERSION)/.dirstamp
...
httpstan/include/lib/boost_$(BOOST_VERSION): build/math-$(MATH_VERSION)/.dirstamp
httpstan/include/lib/eigen_$(EIGEN_VERSION): build/math-$(MATH_VERSION)/.dirstamp
httpstan/include/lib/sundials_$(SUNDIALS_VERSION): build/math-$(MATH_VERSION)/.dirstamp
...
$(SUNDIALS_LIBRARIES_BUILD_LOCATIONS): build/math-$(MATH_VERSION)/.dirstamp
